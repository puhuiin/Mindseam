...
| `history --keep` failed-rotation warning (noun agrees with count) | r290: r214 gave the destructive `history --keep` rotation an honesty branch — when `atomic_write_text` cannot persist the truncated history, the run keeps the full in-memory list and warns on stderr rather than presenting a truncated view it never wrote. That warning's second line hardcoded the plural stem `"reports the full %d rows." % len(hist)`, so a one-row history whose rotation write fails read "reports the full 1 rows." A one-row ledger reaches it deterministically: `history --keep 0` with 1 row satisfies the `len(hist) > keep_n` guard (`1 > 0`), attempts the write, fails, and lands in the honesty branch with `len(hist) == 1`. This is the singular/plural family of r281-r289 on a new surface — a stderr WARNING attached to an I/O-failure branch, not a happy-path human or machine face — showing the family is not confined to normal-path projectors: any face that renders a count, including diagnostics on write-failure paths, must agree with it. The fix pluralizes the noun on the same count (`"" if len(hist) == 1 else "s"`), matching the humanize idiom the siblings use; the r214 honesty contract (keep the full list, report the full count, refuse negative `--keep` with exit 2) is unchanged |
