...
<python-command> <skill-root>/scripts/mindseam.py seam --from-stdin                   # read one next action per line from standard input (like kubectl apply -f - / xargs); r203: under --dry-run the JSON warning reads "N next actions would be recorded", the conditional tense matching the r183 write-nothing preview (a real run keeps the past-tense "recorded" wording); r210: the whole batch lands with ONE history write (an interrupted batch never leaves a partial commit); r289: the JSON warning pluralizes the noun on the count too, so a single piped line reads "1 next action recorded" — agreeing with the text face "From stdin: 1 next action recorded." (0 and 2+ stay "next actions")
<python-command> <skill-root>/scripts/mindseam.py info                                       # aggregate digest of the workspace state; r287: the "History: N entries" line reads "1 entry" (singular) for a one-row history, plural for every other count — the first irregular plural of the r281-r285 singular/plural family (entry->entries, not a bare +s), shared with the history header through one _entries_noun chokepoint so the two faces never disagree; the --json history_count integer carries no noun and is unchanged
...
<python-command> <skill-root>/scripts/mindseam.py info --version                          # print the controller version on its own (like gh --version / kubectl version)
<python-command> <skill-root>/scripts/mindseam.py info --memory                        # report workspace disk size in human units (like free -m / du -h); r285: the size word and its raw byte parenthetical read `1 byte` (singular) for a one-byte workspace, plural for every other count; r286: each KB/MB/GB rung is chosen by the rounded value the reader sees (float("%.1f" % value) < 1024.0), so a size in a rung's top sliver like 1048540 bytes reads "1.0 MB" the way ls -lh promotes it, never "1024.0 KB"
...
<python-command> <skill-root>/scripts/mindseam.py info --mtime                           # carry a workspace_files block listing each ledger artefact (WORKSPACE.md / history.json / metacognition.json / skillbook.md) with mtime, size, presence (like find -printf with T mtime, size, path / stat); r285: a one-byte artefact's size reads `1 byte` (singular), plural for every other count
...
<python-command> <skill-root>/scripts/mindseam.py history                                    # tail the seam audit log (like git log); r287: the header count "(N entries)" reads "(1 entry)" (singular) for a one-row window and plural otherwise — including a longer log narrowed to one row, since the count is len(hist) after filtering; it shares the _entries_noun chokepoint with the info "History:" line so both agree, and the --json history_count is a raw integer
<python-command> <skill-root>/scripts/mindseam.py history --head 5                                # first 5 entries only (like head -n 5); r208: exclusive with --tail and the -n alias (exit 2) — the if/elif used to let head silently win; r275: the head/tail truncation now runs AFTER --filter/--since/--until/--grep/--exclude (it ran ABOVE the window and grep filters), so --head 2 --since 30m keeps the first two rows WITHIN the window (like git log -n 2 --since / journalctl --since -n 2) instead of slicing the two oldest raw rows and letting --since drop them to an empty result at exit 0; --reverse still applies last as presentation
<python-command> <skill-root>/scripts/mindseam.py history --tail 5                                # last 5 entries only (like tail -n 5, alias of -n); r208: exclusive with --head and the -n alias (exit 2) — the alias sharing the tail variable used to win silently; r272: a zero window empties (like tail -n 0, which prints nothing) — the tail slice used to be hist[-tail_n:] if hist else [], and hist[-0:] is hist[0:], the whole list, so --tail 0 (and its -n alias) returned every row at exit 0 while --head 0 correctly emptied; the slice now guards tail_n (hist[-tail_n:] if (hist and tail_n) else []) so the zero window empties the way the --head and --keep siblings do, and the r217 negative refusal (exit 2) is untouched
...
<python-command> <skill-root>/scripts/mindseam.py history --domains                     # group by the next-action domain prefix (like JIT-Agent's diversity analysis); a domain label that reads like an instruction is framed [untrusted: ...] on both faces; r261: the domain label is model-authored ledger text and the ranked line prints it followed by the r252 tag on one print, but the label is next.split(":", 1)[0].strip().lower() and .strip() trims only the ends, so an interior carriage return or newline survived into the label and split the ranked line across two physical lines, stranding the tag on the last — the label now passes through _oneline so one label is one physical line with its tag (a clean label stays byte-identical, the tag scans the raw name, and the JSON face keeps the raw bytes as the recovery path); r281: the non-empty text header read "%d domains across %d seams" % (len(counts), total), but total counts rows WITH a next action (a blank-next row is skipped by the if not nxt guard), not seams — so a window holding a blank-next seam said "across 2 seams" while the history count reported 3, and the noun disagreed with this command's own empty face ("no rows with a next action"); the header now names the ranked unit "next action" and pluralizes both nouns ("1 domain across 2 next actions", "2 domains across 3 next actions", "1 domain across 1 next action") the way discover pluralizes "%d visit%s", and the machine (JSON) face never carried this header and is unchanged
<python-command> <skill-root>/scripts/mindseam.py history --span                        # first seam, last seam and duration (like git log stat); r273: the span is the time EXTENT of the surviving window, so its endpoints are the earliest and latest timestamps (min/max over the rows), not the positional first/last row — --reverse walks the same rows newest-first, which once flipped them so a max(0, last-first) floor reported Duration 0 for a window that plainly spanned time (a hand-written history.json whose rows are not in ascending t hit the same lie without --reverse); min/max make the interval order-invariant the way git log stat reports the same diffstat whatever the walk order, and the floor is dropped because min <= max can never go negative; r283: both nouns of the Duration line pluralize (a one-second window reads "1 second", a one-row window "across 1 row"), matching the discover/domains/dedup family, while the duration stays raw seconds so the machine face and the 9000-second text line are unchanged
...
<python-command> <skill-root>/scripts/mindseam.py history --human                          # relative row ages, the way git log prints relative dates; r284: the shared _humanize_seconds ladder treats a month as 30 days and a year as 365, so 12 months (360 days) is short of a full year — the old months<12 handoff fell through to a years=days//365 branch that was still 0 in [360,365) days, printing "0 years"; the handoff now gates on the year count, so that dead zone reads "12 months" and only [360,365) changes (days>=365 stay "1 year"+ and days<360 keep their month label)
...
<python-command> <skill-root>/scripts/mindseam.py history --keep 500                      # discard older rows and persist the slimmed file (like logrotate --keep, docker system prune); r214: a failed write never fakes the result — the run keeps the full list and warns on stderr that the on-disk history is unchanged (and a negative keep refuses with exit 2, not a silent no-op); r290: that failed-rotation warning now pluralizes its row count ("reports the full 1 row." not "1 rows."), the same singular/plural agreement the history/dedup faces (r282) already carry, so even the I/O-failure diagnostic reads correctly for a one-row ledger
<python-command> <skill-root>/scripts/mindseam.py history --dedup                        # collapse rows to unique next actions (like sort -u / uniq); r274: a terminal renderer — one of eight mutually exclusive renderers, so a combined call is refused with exit 2 (the dedup pair still composes with each other, and the machine face rides --json); r277: that --json face now carries the r245 untrusted map keyed to the deduped rows it ships (two identical override rows fold to one map entry), so a planted SYSTEM OVERRIDE:/ignore-previous surviving the collapse is framed — a clean window yields {}; r282: both text headers now pluralize both nouns ("1 unique next action across 1 row" instead of the old hardcoded "1 unique next actions across 1 rows"), the same singular/plural the siblings --domains (r281) and discover ("%d visit%s") already carry; the blank-next row stays a listed, counted bucket (a blank next is a value --dedup collapses on), so the r277 --json untrusted map still frames a planted SYSTEM OVERRIDE:/ignore-previous carried on a blank-next row; the --json face never carried these headers and is unchanged
<python-command> <skill-root>/scripts/mindseam.py history --dedup-by-msg                # collapse rows to unique msg annotations (like sort -u -k 2); r274: a terminal renderer — one of eight mutually exclusive renderers, so a combined call is refused with exit 2 (the dedup pair still composes with each other)
<python-command> <skill-root>/scripts/mindseam.py history --row-id 3                     # return the single row at the 1-based index N (like git log skip N -n 1); r263: the detail face prints one FIELD per line (next:/msg:), so each field and its [untrusted] tag stay on one physical line even when the value carries any of the eleven splitlines breaks — the --json row keeps the raw bytes for recovery; r276: --row-id indexes the WHOLE log (1..N), so it refuses to compose with any narrowing or reordering flag (--filter, --since/--until, --grep/--exclude, --head/--tail, -n, --reverse, --keep) with exit 2 — those change which rows exist or their order, so the index would silently address a different row; run the narrowing query first, then --row-id its output (the audit --at locator refuses --since/--until and --baseline-write the same way). --row-id still composes with the renderers (--json/--human/--quiet/--count) since those only reshape the one row it names
<python-command> <skill-root>/scripts/mindseam.py history --empty                        # keep only the rows whose next action is blank (like find -empty / awk '/^$/'); r274: a terminal renderer — one of eight mutually exclusive renderers, so a combined call is refused with exit 2 (the machine face rides --json); r277: that --json face now carries the r245 untrusted map keyed to the empty-next slice it ships, so an override riding the msg of a blank-next row is framed — a clean window yields {}; r278: --empty is a CONTENT filter (the sibling of --grep/--exclude/--since), so its predicate now runs in the filter chain BEFORE the r275 head/tail truncation — history --empty --tail 2 keeps the two newest of the blank-next rows (filter-then-truncate, the git log --grep X -n 2 order) instead of slicing the raw window first and finding no empties among the newest rows
...
<python-command> <skill-root>/scripts/mindseam.py discover                                          # modules / domains selected for the next pass; a domain label (and the suggested-next line) that reads like an instruction is framed [untrusted: ...]; r261: both the ranked line and the "Suggested next pass" line that points a host straight at the top domain now run the label through _oneline, so a label carrying a carriage return or newline is one physical line with its tag rather than a two-line split that stranded the tag off the recommendation (a clean label stays byte-identical; the JSON face keeps the raw bytes); r279: discover now groups by the SAME rule as history --domains (nxt.split(":", 1)[0] with an empty prefix bucketed to "(none)") instead of dropping every next with no colon, so a session that recorded bare actions ("refactor the loop") has those rows counted and suggested_next no longer names a colon'd domain while an equally- or more-visited bare action stays invisible; r280: the empty-ranking text face split the two states it collapsed — a truly empty history keeps "No history yet — run a seam and the domain map appears.", but a history that HAS recorded seams with no next action to rank now prints "No next actions recorded yet — note a next and the domain map appears." instead of falsely claiming "No history yet", matching the accurate cause that history --domains already reported ("no rows with a next action"); the --json face is unchanged ({"domains": []} for both, as with history --domains --json)
