...
<python-command> <skill-root>/scripts/mindseam.py history --head 5                                # first 5 entries only (like head -n 5); r208: exclusive with --tail/--limit (exit 2) — the if/elif used to let head silently win
<python-command> <skill-root>/scripts/mindseam.py history --tail 5                                # last 5 entries only (like tail -n 5, alias of -n); r208: exclusive with --head/--limit (exit 2) — --limit sharing the tail variable used to win silently
