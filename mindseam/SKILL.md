...
<python-command> <skill-root>/scripts/mindseam.py history --dedup                        # collapse rows to unique next actions (like sort -u / uniq); r274: a terminal renderer — one of eight mutually exclusive renderers, so a combined call is refused with exit 2 (the dedup pair still composes with each other, and the machine face rides --json)
...
<python-command> <skill-root>/scripts/mindseam.py history --empty                        # keep only the rows whose next action is blank (like find -empty / awk '/^$/'); r274: a terminal renderer — one of eight mutually exclusive renderers, so a combined call is refused with exit 2 (the machine face rides --json)
