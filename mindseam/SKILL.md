...
<python-command> <skill-root>/scripts/mindseam.py info --explain info-memory         # print the static doc for one capability id (summary, since, default) and exit, like kubectl explain; reads the built-in feature catalog, so it works in an empty workspace and creates no ledger; unknown ids refuse with exit 2
